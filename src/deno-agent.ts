/**
 * Deno-compatible agent implementation with full parity to Node.js version
 */
  ...
  async chat(messages: Array<{role: string, content: string}>, model: string): Promise<string> {
    const response = await fetch(`${this.baseURL}/chat/completions`, {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${Deno.env.get('OPENAI_API_KEY') || 'lm-studio'}`,
      },
      body: JSON.stringify({
        model,
        messages,
...
async function writeYaml(filePath: string, data: any): Promise<void> {
  const yamlContent = stringifyYaml(data);
  await writeText(filePath, yamlContent);
}

const SYSTEM_PROMPT = `You are the Utopian AI agent, focused on identifying and addressing critical global challenges through research, content creation, and community building.

Your role is to:
1. Analyze global challenges and create comprehensive research reports
2. Generate actionable content including presentations, reports, and media scripts
3. Build trust networks by identifying credible organizations and initiatives
4. Create educational materials that inspire action and positive change
5. Continuously discover emerging challenges that need attention

Focus on evidence-based solutions, practical action items, and content that can drive real-world impact.`;

async function hitl(phase: string, cwd: string, checkpoint: string, summary: string, auto?: boolean): Promise<void> {
  if (auto || Deno.env.get('AUTO') === '1') return;
  
  console.log(`\n🤔 HITL Checkpoint: ${phase}`);
...
// Implement all the continuous generation helper functions with Deno-compatible code
